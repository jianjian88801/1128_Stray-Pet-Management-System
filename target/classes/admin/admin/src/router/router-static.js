import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'

     import users from '@/views/modules/users/list'
    import address from '@/views/modules/address/list'
    import cart from '@/views/modules/cart/list'
    import chongwu from '@/views/modules/chongwu/list'
    import chongwuLingyang from '@/views/modules/chongwuLingyang/list'
    import chongwuyongpin from '@/views/modules/chongwuyongpin/list'
    import chongwuyongpinCommentback from '@/views/modules/chongwuyongpinCommentback/list'
    import chongwuyongpinOrder from '@/views/modules/chongwuyongpinOrder/list'
    import dictionary from '@/views/modules/dictionary/list'
    import liulangdongwujiuzhu from '@/views/modules/liulangdongwujiuzhu/list'
    import liulangdongwujiuzhuCollection from '@/views/modules/liulangdongwujiuzhuCollection/list'
    import liulangdongwujiuzhuLiuyan from '@/views/modules/liulangdongwujiuzhuLiuyan/list'
    import news from '@/views/modules/news/list'
    import yonghu from '@/views/modules/yonghu/list'
    import config from '@/views/modules/config/list'
    import dictionaryChongwu from '@/views/modules/dictionaryChongwu/list'
    import dictionaryChongwuStatus from '@/views/modules/dictionaryChongwuStatus/list'
    import dictionaryChongwuYesno from '@/views/modules/dictionaryChongwuYesno/list'
    import dictionaryChongwuyongpin from '@/views/modules/dictionaryChongwuyongpin/list'
    import dictionaryChongwuyongpinOrder from '@/views/modules/dictionaryChongwuyongpinOrder/list'
    import dictionaryChongwuyongpinOrderPayment from '@/views/modules/dictionaryChongwuyongpinOrderPayment/list'
    import dictionaryIsdefault from '@/views/modules/dictionaryIsdefault/list'
    import dictionaryLiulangdongwujiuzhuCollection from '@/views/modules/dictionaryLiulangdongwujiuzhuCollection/list'
    import dictionaryNews from '@/views/modules/dictionaryNews/list'
    import dictionarySex from '@/views/modules/dictionarySex/list'
    import dictionaryShangxia from '@/views/modules/dictionaryShangxia/list'





//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    } ,{
        path: '/users',
        name: '管理信息',
        component: users
      }
    ,{
        path: '/dictionaryChongwu',
        name: '宠物类型',
        component: dictionaryChongwu
    }
    ,{
        path: '/dictionaryChongwuStatus',
        name: '认领状态',
        component: dictionaryChongwuStatus
    }
    ,{
        path: '/dictionaryChongwuYesno',
        name: '认领状态',
        component: dictionaryChongwuYesno
    }
    ,{
        path: '/dictionaryChongwuyongpin',
        name: '商品类型',
        component: dictionaryChongwuyongpin
    }
    ,{
        path: '/dictionaryChongwuyongpinOrder',
        name: '订单类型',
        component: dictionaryChongwuyongpinOrder
    }
    ,{
        path: '/dictionaryChongwuyongpinOrderPayment',
        name: '订单支付类型',
        component: dictionaryChongwuyongpinOrderPayment
    }
    ,{
        path: '/dictionaryIsdefault',
        name: '是否默认地址',
        component: dictionaryIsdefault
    }
    ,{
        path: '/dictionaryLiulangdongwujiuzhuCollection',
        name: '收藏表类型',
        component: dictionaryLiulangdongwujiuzhuCollection
    }
    ,{
        path: '/dictionaryNews',
        name: '公告类型',
        component: dictionaryNews
    }
    ,{
        path: '/dictionarySex',
        name: '性别类型',
        component: dictionarySex
    }
    ,{
        path: '/dictionaryShangxia',
        name: '上下架',
        component: dictionaryShangxia
    }
    ,{
        path: '/config',
        name: '轮播图',
        component: config
    }


    ,{
        path: '/address',
        name: '送货地址',
        component: address
      }
    ,{
        path: '/cart',
        name: '购物车',
        component: cart
      }
    ,{
        path: '/chongwu',
        name: '流浪宠物',
        component: chongwu
      }
    ,{
        path: '/chongwuLingyang',
        name: '流浪宠物领养',
        component: chongwuLingyang
      }
    ,{
        path: '/chongwuyongpin',
        name: '商品',
        component: chongwuyongpin
      }
    ,{
        path: '/chongwuyongpinCommentback',
        name: '商品评价',
        component: chongwuyongpinCommentback
      }
    ,{
        path: '/chongwuyongpinOrder',
        name: '商品订单',
        component: chongwuyongpinOrder
      }
    ,{
        path: '/dictionary',
        name: '字典',
        component: dictionary
      }
    ,{
        path: '/liulangdongwujiuzhu',
        name: '流浪动物救助',
        component: liulangdongwujiuzhu
      }
    ,{
        path: '/liulangdongwujiuzhuCollection',
        name: '流浪动物救助收藏',
        component: liulangdongwujiuzhuCollection
      }
    ,{
        path: '/liulangdongwujiuzhuLiuyan',
        name: '流浪动物救助留言',
        component: liulangdongwujiuzhuLiuyan
      }
    ,{
        path: '/news',
        name: '公告信息',
        component: news
      }
    ,{
        path: '/yonghu',
        name: '用户',
        component: yonghu
      }


    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
